...
class QasmParser:
    ...
    def verify_declared_bit(self, obj):
        ...
        if obj.name not in self.current_symtab:
            raise QasmError(
                "Cannot find symbol '" + obj.name + "' in argument list for gate, line", str(obj.line), "file", obj.file
            )
            ...
            for children in obj.children:
                if isinstance(children, node.Id):
                    ...
                    if children.name not in self.current_symtab:
                        raise QasmError(
                            "Argument '" + children.name + "' in expression cannot be " + "found, line",
                            str(children.line),
                            "file",
                            children.file,
                        )
    ...
    def verify_as_gate(self, obj, bitlist, arglist=None):
        ...
        if arglist:
            ...
            if g_sym.n_args() > 0:
                raise QasmError(
                    "Gate or opaque call to '" + obj.name + "' has no arguments but is declared for",
                    str(g_sym.n_args()),
                    "qubits",
                    "line",
                    str(obj.line),
    ...
    def p_decl(self, program):
        ...
        if len(program) > 2:
            if program[2] != ";":
                raise QasmError(
                    "Missing ';' in qreg or creg declaration." " Instead received '" + program[2].value + "'"
                )
        program[0] = program[1]
    ...
    def p_gate_op_1e1(self, program):
        ...
        """
        raise QasmError(
            "Invalid CX inside gate definition. " + "Expected an ID or ',', received '" + str(program[2].value) + "'"
        )
    ...
    def p_gate_op_1e2(self, program):
        ...
        """
        raise QasmError(
            "Invalid CX inside gate definition. " + "Expected an ID or ';', received '" + str(program[4].value) + "'"
        )
    ...
    def get_tokens(self):
        ...
        warnings.warn("The method get_tokens() is being replaced by read_tokens()", DeprecationWarning, 2)
        try:
            while True:
