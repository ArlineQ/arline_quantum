...
CORE_LIBS_PATH = os.path.join(os.path.dirname(__file__), "qasm_header")
...
class QasmLexer:
    ...
    def t_INCLUDE(self, t):
        ...
        if not os.path.exists(incfile):
            raise QasmError(
                "Include file %s cannot be found, line %s, file %s" % (incfile, str(next_token.lineno), self.filename)
            )
        self.push(incfile)
